...
跨 IDE的 AI 编程助手，创新的悬浮球模式使得你不再需要在各个IDE之间切换各种编程插件，也不会被各种对话框界面占用大量的屏幕空间，配置任意兼容 OpenAI格式的模型服务，打开项目目录，即可开始编码会话。
...
由于当前版本对Anthropic API的兼容性不佳（基本不可用）建议使用OpenAI以及兼容 OpenAI 的接口
